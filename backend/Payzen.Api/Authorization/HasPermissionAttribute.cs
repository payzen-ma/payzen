...
public class HasPermissionAttribute : Attribute, IAuthorizationFilter
{
    ...
    public void OnAuthorization(AuthorizationFilterContext context)
    {
        ...
        {
            context.Result = new UnauthorizedObjectResult(new { Message = "Authentification requise" });
            return;
        }
        ...
        if (!hasPermission)
        {
            context.Result = new ObjectResult(
                new { Message = "Vous n'avez pas les permissions nécessaires", RequiredPermissions = _permissions }
            )
            {
