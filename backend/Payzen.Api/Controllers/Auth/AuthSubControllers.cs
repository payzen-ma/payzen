...
public class UsersController : ControllerBase
    ...
    public UsersController(IAuthService auth, IUserInviteService userInviteService)
        {
        _auth = auth;
    ...
    public async Task<ActionResult> Invite([FromBody] UserInviteDto dto, CancellationToken ct)
        ...
        var result = await _userInviteService.InviteUserAsync(dto, User.GetUserId(), ct);
        if (!result.Success)
            {
            if (result.Error == "Company introuvable.")
                return NotFound(new
                    {
