...
public class EmployeeController : ControllerBase
{
    ...
    public async Task<ActionResult> GetFormData([FromQuery] int? companyId)
    {
        ...
        if (!r.Success)
        {
            if (r.Error == "Société cible non trouvée.")
                return NotFound(new { Message = r.Error });
            if (r.Error == "Accès refusé à cette société.")
                return Forbid();
...
public class AbsencesController : ControllerBase
{
    ...
    {
        var r = await _absences.GetByCompanyAsync(
            companyId,
            employeeId,
            startDate,
            endDate,
        ...
        if (!r.Success)
        {
            if (r.Error == "Société non trouvée")
                return NotFound(new { Message = r.Error });
            if (r.Error?.Contains("Employé") == true)
                return NotFound(new { Message = r.Error });
            return BadRequest(new { Message = r.Error });
        }
        return Ok(r.Data);
    ...
    public async Task<ActionResult> GetAll(int employeeId, [FromQuery] int? year, [FromQuery] int? month)
    {
        ...
        var to =
            (year.HasValue && month.HasValue)
                ? new DateOnly?(new DateOnly(year.Value, month.Value, DateTime.DaysInMonth(year.Value, month.Value)))
                : null;
...
public class EmployeeOvertimesListController : ControllerBase
{
    ...
    public ActionResult GetOvertimeTypeCatalog()
    {
        var types = new List<object>
        {
            ...
            new
            {
                Value = (int)OvertimeType.Standard,
                Code = "Standard",
                DescriptionFr = "Standard",
                DescriptionEn = "Standard",
            },
            new
            {
                Value = (int)OvertimeType.WeeklyRest,
                Code = "WeeklyRest",
                DescriptionFr = "Repos hebdomadaire",
                DescriptionEn = "Weekly Rest",
            },
            new
            {
                Value = (int)OvertimeType.PublicHoliday,
                Code = "PublicHoliday",
                DescriptionFr = "Jour férié",
                DescriptionEn = "Public Holiday",
            },
            new
            {
                Value = (int)OvertimeType.Night,
                Code = "Night",
                DescriptionFr = "Nuit",
                DescriptionEn = "Night",
            },
            new
            {
                Value = (int)(OvertimeType.Standard | OvertimeType.Night),
                Code = "Standard_Night",
                DescriptionFr = "Standard + Nuit",
                DescriptionEn = "Standard + Night",
            },
            new
            {
                Value = (int)(OvertimeType.WeeklyRest | OvertimeType.Night),
                Code = "WeeklyRest_Night",
                DescriptionFr = "Repos hebdomadaire + Nuit",
                DescriptionEn = "Weekly Rest + Night",
            },
            new
            {
                Value = (int)(OvertimeType.PublicHoliday | OvertimeType.Night),
                Code = "PublicHoliday_Night",
                DescriptionFr = "Jour férié + Nuit",
                DescriptionEn = "Public Holiday + Night",
            },
            new
            {
                Value = (int)OvertimeType.FerieOrRest,
                Code = "FerieOrRest",
                DescriptionFr = "Férié ou Repos",
                DescriptionEn = "Holiday or Rest",
            },
        };
        return Ok(types);
    }
...
public class EmployeeOvertimeController : ControllerBase
{
    ...
    public async Task<ActionResult> Create(int employeeId, [FromBody] EmployeeOvertimeCreateDto dto)
    {
        ...
        if (outcome.Overtimes.Count == 1)
            return CreatedAtRoute(
                "GetEmployeeOvertimeByEmployeeAndId",
                new { employeeId, id = outcome.Overtimes[0].Id },
                outcome.Overtimes[0]
            );
        return Ok(
            new
