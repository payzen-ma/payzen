...
public class ContractTypesController : ControllerBase
{
    ...
    public async Task<ActionResult> GetAll([FromQuery] int? companyId)
    {
        if (!companyId.HasValue)
            return BadRequest(new { Message = "companyId requis." });
...
public class WorkingCalendarsController : ControllerBase
{
    ...
    [HttpPut("{id:int}")]
    public Task<ActionResult> Put(int id, [FromBody] WorkingCalendarUpdateDto dto) => UpdateWorkingCalendar(id, dto);

    [HttpPatch("{id:int}")]
    public Task<ActionResult> Patch(int id, [FromBody] WorkingCalendarUpdateDto dto) => UpdateWorkingCalendar(id, dto);
    ...
    private async Task<ActionResult> UpdateWorkingCalendar(int id, WorkingCalendarUpdateDto dto)
    {
        ...
        if (!r.Success)
        {
            if (r.Error == "Calendrier de travail non trouvé")
                return NotFound(new { Message = r.Error });
            if (r.Error == "Société non trouvée")
                return NotFound(new { Message = r.Error });
            if (r.Error == "Un calendrier existe déjà pour ce jour de la semaine dans cette société")
