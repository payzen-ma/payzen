...
public class PayrollController : ControllerBase
{
    ...
    )
    {
        ...
        if (dto == null)
            return BadRequest(
                new
                {
                    error = "Requête invalide : indiquez companyId, month et year en query, ou un corps JSON avec employeeId, payMonth et payYear.",
                }
            );
    ...
    )
    {
        ...
        var r = await _svc.GetResultsAsync(companyId is > 0 ? companyId : null, month, year, payHalf, status);
    ...
    public async Task<ActionResult> DeleteResult(int id)
    {
        var r = await _svc.DeleteResultAsync(id, User.GetUserId());
        return r.Success
            ? Ok(new { message = "Résultat de paie supprimé avec succès." })
...
public class SalaryPreviewController : ControllerBase
{
    ...
    public ActionResult GetSeniorityBonus() =>
        Ok(
            new
            {
                description = "Prime d'ancienneté : 5% (2-5 ans), 10% (5-12 ans), 15% (12-20 ans), 20% (20+ ans)",
                ruleVersion = "MA_2025",
            }
        );

    [HttpGet("cnss")]
    public ActionResult GetCnss() =>
        Ok(new { description = "Cotisation CNSS employé/employeur selon plafond", ruleVersion = "MA_2025" });
    ...
    public ActionResult GetProfessionalExpenses() =>
        Ok(new { description = "Frais professionnels déductibles (transport, panier)", ruleVersion = "MA_2025" });
}
...
public class PayrollExportController : ControllerBase
{
    ...
    public async Task<ActionResult> EtatCnss(int companyId, [FromQuery] int year, [FromQuery] int month)
    {
    ...
    public async Task<ActionResult> EtatIr(int companyId, [FromQuery] int year, [FromQuery] int month)
    {
...
public class PayslipController : ControllerBase
{
    ...
    public async Task<ActionResult> GenerateByEmployeePeriod(int employeeId, int year, int month, [FromQuery] int? half)
    {
    ...
    public async Task<ActionResult> CnssPdf(int companyId, [FromQuery] int year, [FromQuery] int month)
    {
    ...
    public async Task<ActionResult> IrPdf(int companyId, [FromQuery] int year, [FromQuery] int month)
    {
...
public class PayrollExportsCompatController : ControllerBase
{
    ...
    private static bool ValidatePeriod(int companyId, int year, int month, out string? error)
    {
        ...
        if (companyId <= 0)
        {
            error = "companyId invalide.";
            return false;
        }
        if (month is < 1 or > 12)
        {
            error = "month invalide (1-12).";
            return false;
        }
    ...
    public async Task<IActionResult> DownloadCnss(int companyId, int year, int month, CancellationToken ct)
    {
        ...
        {
            sb.Append(r.NomPrenom)
                .Append(';')
                .Append(r.NumeroCnss)
                .Append(';')
                .Append(r.SalaireBrutDeclare.ToString("F2", fr))
                .Append(';')
    ...
    public async Task<IActionResult> DownloadIr(int companyId, int year, int month, CancellationToken ct)
    {
        ...
        foreach (var r in rows.Data)
        {
            sb.Append(r.NomPrenom)
                ...
                .Append(r.CNSS)
                .Append(';')
                .Append(r.BrutImposable.ToString("F2", fr))
                .Append(';')
...
public class ClaudeSimulationController : ControllerBase
{
    ...
    [HttpPost]
    public async Task<ActionResult> Simulate([FromBody] PayrollSimulateRequestDto dto, [FromQuery] string? instruction)
    {
        var payrollResult = await _payroll.SimulateAsync(dto);
        if (!payrollResult.Success)
            return BadRequest(new { Message = payrollResult.Error });
        ...
        var analyse = await _llm.SimulationSalaryAsync(rules, userMsg);

        return Ok(new { simulation = data, analysis = analyse });
    }

    [HttpPost("simulate-quick")]
    public async Task<ActionResult> SimulateQuick(
        [FromBody] PayrollSimulateRequestDto dto,
    ...
    public async Task<ActionResult> SimulateStream(
        [FromBody] PayrollSimulateRequestDto dto,
        [FromQuery] string? instruction
    )
    {
