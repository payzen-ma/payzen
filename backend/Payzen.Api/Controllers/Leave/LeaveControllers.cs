...
public class LeaveTypeController : ControllerBase
{
    ...
    public async Task<ActionResult> Put(int id, [FromBody] LeaveTypePatchDto dto)
    {
        ...
        if (!r.Success)
        {
            if (r.Error == "Type de congé introuvable.") return NotFound(new { Message = r.Error });
            if (r.Error?.Contains("existe déjà") == true) return Conflict(new { Message = r.Error });
            return BadRequest(new { Message = r.Error });
        }
        return Ok(r.Data);
...
public class LeaveTypePolicyController : ControllerBase
{
    ...
    public async Task<ActionResult> Create([FromBody] LeaveTypePolicyCreateDto dto)
    {
        ...
        if (!r.Success)
        {
            if (r.Error == "Type de congé non trouvé.") return NotFound(new { Message = r.Error });
            if (r.Error?.Contains("existe déjà") == true) return Conflict(new { Message = r.Error });
            return BadRequest(new { Message = r.Error });
        }
        return Ok(r.Data);
    ...
    public async Task<ActionResult> Put(int id, [FromBody] LeaveTypePolicyPatchDto dto)
    {
        ...
        if (!r.Success)
        {
            if (r.Error == "Politique introuvable.") return NotFound(new { Message = r.Error });
            return BadRequest(new { Message = r.Error });
        }
        return Ok(r.Data);
    }
...
public class LeaveTypeLegalRuleController : ControllerBase
{
    ...
    public async Task<ActionResult> Put(int id, [FromBody] LeaveTypeLegalRulePatchDto dto)
    {
        ...
        if (!r.Success)
        {
            if (r.Error == "Règle légale introuvable.") return NotFound(new { Message = r.Error });
            if (r.Error?.Contains("existe déjà") == true) return Conflict(new { Message = r.Error });
            return BadRequest(new { Message = r.Error });
        }
        return Ok(r.Data);
...
public class LeaveRequestController : ControllerBase
{
    ...
    public async Task<ActionResult> Put(int id, [FromBody] LeaveRequestPatchDto dto)
    {
        ...
        if (!r.Success)
        {
            if (r.Error == "Demande introuvable.") return NotFound(new { Message = r.Error });
            return BadRequest(new { Message = r.Error });
        }
        return Ok(r.Data);
    }
...
public class LeaveBalanceController : ControllerBase
{
    ...
    public async Task<ActionResult> Create([FromBody] LeaveBalanceCreateDto dto)
    {
        ...
        if (!r.Success)
        {
            if (r.Error == "Employé non trouvé." || r.Error == "Type de congé non trouvé.") return NotFound(new { Message = r.Error });
            if (r.Error?.Contains("existe déjà") == true) return Conflict(new { Message = r.Error });
            return BadRequest(new { Message = r.Error });
        }
    ...
    public async Task<ActionResult> Put(int id, [FromBody] LeaveBalancePatchDto dto)
    {
        ...
        if (!r.Success)
        {
            if (r.Error == "Solde introuvable.") return NotFound(new { Message = r.Error });
            return BadRequest(new { Message = r.Error });
        }
        return Ok(r.Data);
    }
    ...
    public async Task<ActionResult> Recalculate(
        ...
        [FromQuery] int? leaveTypeId)
    {
        if (month < 1 || month > 12) return BadRequest(new { Message = "Le mois doit être entre 1 et 12." });
        var r = await _svc.RecalculateBalancesForMonthAsync(employeeId, year, month, companyId, leaveTypeId, User.GetUserId());
        if (!r.Success)
        {
        ...
        var d = r.Data!;
        return Ok(new { Message = d.Message, RecalculatedCount = d.RecalculatedCount, TotalBalances = d.TotalBalances });
    }
...
public class LeaveCarryOverAgreementController : ControllerBase
{
    ...
    {
        if (!employeeId.HasValue) return BadRequest(new { Message = "employeeId est requis." });
        ...
        if (companyId.HasValue) data = data.Where(a => a.CompanyId == companyId.Value);
    ...
    public async Task<ActionResult> Put(int id, [FromBody] LeaveCarryOverAgreementPatchDto dto)
    {
        ...
        if (!r.Success)
        {
            if (r.Error == "Accord introuvable.") return NotFound(new { Message = r.Error });
            return BadRequest(new { Message = r.Error });
        }
        return Ok(r.Data);
    }
...
public class LeaveRequestAttachmentController : ControllerBase
{
        ...
        [FromForm] IFormFile file)
    {
        ...
        if (!r.Success)
        {
            ...
            if (r.Error == "Demande de congé non trouvée.") return NotFound(new { Message = r.Error });
            return BadRequest(new { Message = r.Error });
        }
    ...
    public async Task<IActionResult> Download(int id)
    {
        ...
        if (content.Length == 0) return NotFound(new { Message = "Fichier physique non trouvé sur le serveur." });
    ...
    public async Task<ActionResult> Delete(int id)
    {
        ...
        if (!r.Success)
        {
            if (r.Error == "Pièce jointe introuvable.") return NotFound(new { Message = r.Error });
            return BadRequest(new { Message = r.Error });
        }
        return NoContent();
    }
...
public class LeaveRequestExemptionController : ControllerBase
{
    ...
    public async Task<ActionResult> Put(int id, [FromBody] LeaveRequestExemptionPatchDto dto)
    {
        ...
        if (!r.Success)
        {
            if (r.Error == "Exemption introuvable.") return NotFound(new { Message = r.Error });
            return BadRequest(new { Message = r.Error });
        }
        return Ok(r.Data);
    }
