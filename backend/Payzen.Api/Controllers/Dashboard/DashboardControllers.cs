...
public class DashboardController : ControllerBase
{
    ...
    public async Task<ActionResult> GetEmployees()
    {
        ...
        if (!r.Success)
        {
            if (r.Error == "Utilisateur non trouvé") return NotFound(new { Message = r.Error });
            if (r.Error == "L'utilisateur n'est pas associé à un employé") return BadRequest(new { Message = r.Error });
