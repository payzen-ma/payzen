...
public class SalaryPackageCreateDto
{
    ...
    [RegularExpression(
        ...
        ErrorMessage = "Status must be one of: draft, published, deprecated"
    )]
...
public class SalaryPackageItemWriteDto
{
    ...
    [RegularExpression(
        "^(base_salary|allowance|bonus|benefit_in_kind|social_charge)$",
...
public class PayComponentWriteDto
{
    ...
    [RegularExpression(
        "^[A-Z0-9_]+$",
        ErrorMessage = "Code must contain only uppercase letters, numbers, and underscores"
    )]
