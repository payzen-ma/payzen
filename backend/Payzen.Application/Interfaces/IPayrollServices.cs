...
public interface IPayrollService
{
    ...
    Task<ServiceResult<PayrollResultReadDto>> CalculateAsync(
    ...
    Task<ServiceResult<PayrollResultReadDto>> SimulateAsync(
        PayrollSimulateRequestDto dto,
        CancellationToken ct = default
    );

    /// <summary>Calcule la paie de tous les employés actifs d'une société pour un mois.</summary>
    Task<ServiceResult<IEnumerable<PayrollResultReadDto>>> BatchCalculateAsync(
    ...
    Task<ServiceResult<PayrollBulletinResultsResponseDto>> GetResultsAsync(
        int? companyId,
        int month,
        int year,
        int? payHalf,
    ...
    Task<ServiceResult<PayrollResultReadDto>> RecalculateForEmployeeAsync(
        int employeeId,
        int month,
        int year,
        int? payHalf,
