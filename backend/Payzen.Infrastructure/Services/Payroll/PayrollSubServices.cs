...
public class PayComponentService : IPayComponentService
{
        ...
        bool? isActive, CancellationToken ct = default)
    {
        ...
        if (isActive.HasValue) q = q.Where(pc => pc.IsActive == isActive.Value);
    ...
    {
        var pc = await _db.PayComponents.FindAsync(new object[] { id }, ct);
        if (pc == null) return ServiceResult<PayComponentReadDto>.Fail("Composante introuvable.");
    ...
    public async Task<ServiceResult<PayComponentReadDto>> NewVersionAsync(int id, int userId, CancellationToken ct = default)
    {
        ...
        if (src == null) return ServiceResult<PayComponentReadDto>.Fail("Composante introuvable.");
        ...
        _db.PayComponents.Add(next); await _db.SaveChangesAsync(ct);
    ...
    public async Task<ServiceResult<PayComponentReadDto>> DeactivateAsync(int id, int userId, CancellationToken ct = default)
    {
        var pc = await _db.PayComponents.FindAsync(new object[] { id }, ct);
        if (pc == null) return ServiceResult<PayComponentReadDto>.Fail("Composante introuvable.");
        pc.IsActive = false; pc.UpdatedBy = userId; pc.UpdatedAt = DateTimeOffset.UtcNow;
        await _db.SaveChangesAsync(ct);
        return ServiceResult<PayComponentReadDto>.Ok(Map(pc));
    }
    ...
    {
        var pc = await _db.PayComponents.FindAsync(new object[] { id }, ct);
        if (pc == null) return ServiceResult.Fail("Composante introuvable.");
...
public class ReferentielPayrollService : IReferentielPayrollService
{
        ...
        bool? isActive, CancellationToken ct = default)
    {
        var q = _db.ReferentielElements
            ...
            .Include(re => re.Rules)
            .AsQueryable();
        if (isActive.HasValue) q = q.Where(re => re.IsActive == isActive.Value);
    ...
    public async Task<ServiceResult<ReferentielElementDto>> GetElementByIdAsync(int id, CancellationToken ct = default)
    {
        ...
        if (re == null) return ServiceResult<ReferentielElementDto>.Fail("Élément introuvable.");
        ...
        CreateReferentielElementDto dto, int createdBy, CancellationToken ct = default)
    {
        ...
        return ServiceResult<ReferentielElementDto>.Ok(new ReferentielElementDto
        {
            ...
            CategoryId = re.CategoryId, CategoryName = re.Category.Name,
            Description = re.Description, DefaultFrequency = re.DefaultFrequency,
            Status = re.Status, IsActive = re.IsActive
        });
    }
        ...
        int id, UpdateReferentielElementDto dto, int updatedBy, CancellationToken ct = default)
    {
        ...
        if (dto.Name        != null) re.Name        = dto.Name;
        if (dto.Description != null) re.Description = dto.Description;
    ...
    public async Task<ServiceResult> DeleteElementAsync(int id, int deletedBy, CancellationToken ct = default)
    {
        ...
        if (re == null) return ServiceResult.Fail("Élément introuvable.");
        re.DeletedAt = DateTimeOffset.UtcNow; re.DeletedBy = deletedBy;
        ...
        CreateElementRuleDto dto, int createdBy, CancellationToken ct = default)
    {
        ...
        {
            Id = rule.Id, ElementId = rule.ElementId, AuthorityId = rule.AuthorityId,
            AuthorityName = rule.Authority.Name, ExemptionType = rule.ExemptionType,
            SourceRef = rule.SourceRef, EffectiveFrom = rule.EffectiveFrom,
            EffectiveTo = rule.EffectiveTo, IsActive = rule.IsActive()
        });
    }
        ...
        int id, UpdateElementRuleDto dto, int updatedBy, CancellationToken ct = default)
    {
        ...
        if (rule == null) return ServiceResult<ElementRuleDto>.Fail("Règle introuvable.");
        if (dto.EffectiveTo   != null) rule.EffectiveTo   = dto.EffectiveTo;
        if (dto.SourceRef     != null) rule.SourceRef     = dto.SourceRef;
        if (dto.Status        != null) rule.Status        = dto.Status.Value;
    ...
    public async Task<ServiceResult> DeleteRuleAsync(int id, int deletedBy, CancellationToken ct = default)
    {
        ...
        if (rule == null) return ServiceResult.Fail("Règle introuvable.");
        rule.DeletedAt = DateTimeOffset.UtcNow; rule.DeletedBy = deletedBy;
        await _db.SaveChangesAsync(ct); return ServiceResult.Ok();
    }
