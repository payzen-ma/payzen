...
public class PayrollExportService : IPayrollExportService
{
        ...
        int companyId, int year, int month, CancellationToken ct = default)
    {
        ...
        var rows = results.Select(pr => new JournalPaieRow
        {
            // Ligne 1
        ...
        int companyId, int year, int month, CancellationToken ct = default)
    {
        ...
        var rows = results.Select(pr => new EtatCnssRow
        {
        ...
        int companyId, int year, int month, CancellationToken ct = default)
    {
        ...
        if (company == null) return ServiceResult<EtatCnssPdfData>.Fail("Société introuvable.");
        ...
        return ServiceResult<EtatCnssPdfData>.Ok(new EtatCnssPdfData
        {
        ...
        int companyId, int year, int month, CancellationToken ct = default)
    {
        ...
        var rows = results.Select(pr => new EtatIrRow
        {
            NomPrenom    = $"{pr.Employee.LastName} {pr.Employee.FirstName}",
            CIN          = pr.Employee.CinNumber,
            CNSS         = pr.Employee.CnssNumber ?? string.Empty,
    ...
    {
        var company = await _db.Companies.FindAsync(new object[] { companyId }, ct);
        if (company == null) return ServiceResult<EtatIrPdfData>.Fail("Société introuvable.");

        var results = await LoadResultsWithEmployee(companyId, year, month, ct);

        var rows = results.Select(pr => new EtatIrFullRow
        {
        ...
        return ServiceResult<EtatIrPdfData>.Ok(new EtatIrPdfData
        {
            CompanyName    = company.CompanyName,
            CompanyAddress = company.CompanyAddress,
            Month          = month,
    ...
    private async Task<List<Domain.Entities.Payroll.PayrollResult>> LoadResultsWithEmployee(
        int companyId, int year, int month, CancellationToken ct)
        => await _db.PayrollResults
