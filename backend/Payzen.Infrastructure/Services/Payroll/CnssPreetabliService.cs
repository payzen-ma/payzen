...
{
    public async Task<ServiceResult<CnssPreetabliParseResultDto>> ParseAsync(IFormFile file, CancellationToken ct = default)
    {
        if (file.Length == 0)
    ...
    private static void ParseA02(string line, int lineNumber, CnssPreetabliParseResultDto result)
    {
        var row = new CnssPreetabliEmployeeRowDto
        {
            ...
            FamilyAllowanceToPay = ParseCentimesToDecimal(Slice(line, 88, 6)),
            FamilyAllowanceToDeduct = ParseCentimesToDecimal(Slice(line, 94, 6)),
            FamilyAllowanceNetToPay = ParseCentimesToDecimal(Slice(line, 100, 6)),
        };
    ...
    private static void ParseA03(string line, int lineNumber, CnssPreetabliParseResultDto result)
    {
        result.Summary = new CnssPreetabliSummaryDto
        {
            ...
            TotalFamilyAllowanceToPay = ParseCentimesToDecimal(Slice(line, 29, 12)),
            TotalFamilyAllowanceToDeduct = ParseCentimesToDecimal(Slice(line, 41, 12)),
            TotalFamilyAllowanceNetToPay = ParseCentimesToDecimal(Slice(line, 53, 12)),
