...
public class SalaryPackageService : ISalaryPackageService
{
    ...
    public async Task<ServiceResult<IEnumerable<SalaryPackageReadDto>>> GetAllAsync(
        int? companyId, string? scope, string? status, CancellationToken ct = default)
    {
        var q = _db.SalaryPackages
    ...
    public async Task<ServiceResult<IEnumerable<SalaryPackageReadDto>>> GetTemplatesAsync(CancellationToken ct = default)
    {
        var list = await _db.SalaryPackages
            .Where(sp => sp.DeletedAt == null && sp.CompanyId == null)
            .Include(sp => sp.Company)
            .Include(sp => sp.BusinessSector)
            .Include(sp => sp.Items)
            .OrderBy(sp => sp.Name)
    ...
    {
        var sp = await _db.SalaryPackages
            .Include(sp => sp.Company)
            .Include(sp => sp.BusinessSector)
            .Include(sp => sp.Items).ThenInclude(i => i.PayComponent)
            .Include(sp => sp.Items).ThenInclude(i => i.ReferentielElement)
    ...
    public async Task<ServiceResult<SalaryPackageReadDto>> CloneAsync(
        int id, SalaryPackageCloneDto dto, int userId, CancellationToken ct = default)
    {
    ...
    public async Task<ServiceResult<SalaryPackageReadDto>> DuplicateAsync(
        int id, SalaryPackageDuplicateDto dto, int userId, CancellationToken ct = default)
    {
    ...
    public async Task<ServiceResult<IEnumerable<SalaryPackageItemReadDto>>> GetItemsAsync(int packageId, CancellationToken ct = default)
    {
        var items = await _db.SalaryPackageItems.Where(i => i.SalaryPackageId == packageId && i.DeletedAt == null).OrderBy(i => i.SortOrder).Include(i => i.PayComponent).Include(i => i.ReferentielElement).ToListAsync(ct);
        return ServiceResult<IEnumerable<SalaryPackageItemReadDto>>.Ok(items.Select(i => new SalaryPackageItemReadDto { Id = i.Id, PayComponentId = i.PayComponentId, PayComponentCode = i.PayComponent?.Code, ReferentielElementId = i.ReferentielElementId, ReferentielElementName = i.ReferentielElement?.Name, Label = i.Label, DefaultValue = i.DefaultValue, SortOrder = i.SortOrder, Type = i.Type, IsTaxable = i.IsTaxable, IsSocial = i.IsSocial, IsCIMR = i.IsCIMR, IsVariable = i.IsVariable, ExemptionLimit = i.ExemptionLimit }));
    }
    ...
    {
        var list = await _db.SalaryPackageAssignments
    ...
    public async Task<ServiceResult<SalaryPackageAssignmentReadDto>> GetAssignmentByIdAsync(int id, CancellationToken ct = default)
    {
        var a = await _db.SalaryPackageAssignments.Include(x => x.SalaryPackage).Include(x => x.Employee).FirstOrDefaultAsync(x => x.Id == id && x.DeletedAt == null, ct);
        return a == null ? ServiceResult<SalaryPackageAssignmentReadDto>.Fail("Assignation introuvable.") : ServiceResult<SalaryPackageAssignmentReadDto>.Ok(MapAssignment(a));
    }

    public async Task<ServiceResult<SalaryPackageAssignmentReadDto>> AssignAsync(
        SalaryPackageAssignmentCreateDto dto, int createdBy, CancellationToken ct = default)
    {
        // Fermer l'assignation active précédente
        var active = await _db.SalaryPackageAssignments
    ...
    public async Task<ServiceResult<SalaryPackageAssignmentReadDto>> UpdateAssignmentAsync(
        int id, SalaryPackageAssignmentUpdateDto dto, int updatedBy, CancellationToken ct = default)
    {
    ...
    public async Task<ServiceResult<PayrollSummaryDto>> PreviewAsync(
        SalaryPreviewRequestDto dto, CancellationToken ct = default)
    {
        var baseSalary = dto.BaseSalary;
        var allowances = dto.Items
            .Where(i => i.Type == "allowance")
            .Sum(i => i.DefaultValue);
        var bonuses = dto.Items
            .Where(i => i.Type == "bonus")
            .Sum(i => i.DefaultValue);
