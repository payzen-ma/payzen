...
public class LeaveService : ILeaveService
{
    ...
    public async Task<ServiceResult<IEnumerable<LeaveTypeReadDto>>> GetLeaveTypesAsync(
        int? companyId,
    ...
    )
    {
        ...
        var raw = await q.Include(lt => lt.Company).OrderBy(lt => lt.LeaveCode).ToListAsync(ct);
    ...
    public async Task<ServiceResult<LeaveTypeReadDto>> CreateLeaveTypeAsync(
    ...
    )
    {
        if (dto.Scope == LeaveScope.Company && dto.CompanyId == null)
            return ServiceResult<LeaveTypeReadDto>.Fail(
                "CompanyId est obligatoire pour les types de congés d'entreprise (Scope=Company)."
            );
        if (dto.Scope == LeaveScope.Global && dto.CompanyId != null)
            return ServiceResult<LeaveTypeReadDto>.Fail(
                "CompanyId doit être null pour les types de congés globaux (Scope=Global)."
            );

        // Vérification unicité du LeaveCode
        bool codeExists;
        if (dto.Scope == LeaveScope.Global)
            codeExists = await _db.LeaveTypes.AnyAsync(
                lt => lt.LeaveCode == dto.LeaveCode && lt.Scope == LeaveScope.Global && lt.DeletedAt == null,
                ct
            );
        else
            codeExists = await _db.LeaveTypes.AnyAsync(
                lt => lt.LeaveCode == dto.LeaveCode && lt.CompanyId == dto.CompanyId && lt.DeletedAt == null,
                ct
            );

        if (codeExists)
            return ServiceResult<LeaveTypeReadDto>.Fail(
                $"Un type de congé avec le code '{dto.LeaveCode}' existe déjà dans ce contexte."
            );
    ...
    public async Task<ServiceResult<LeaveTypeReadDto>> PatchLeaveTypeAsync(
        int id,
        LeaveTypePatchDto dto,
        int updatedBy,
        CancellationToken ct = default
    )
    {
        var lt = await _db
            .LeaveTypes.Include(x => x.Company)
            .FirstOrDefaultAsync(x => x.Id == id && x.DeletedAt == null, ct);
        if (lt == null)
            ...
            if (lt.Scope == LeaveScope.Global)
                ...
                codeExists = await _db.LeaveTypes.AnyAsync(
                    x =>
                        x.Id != id
                        && x.LeaveCode == dto.LeaveCode
                        && x.CompanyId == lt.CompanyId
                        && x.DeletedAt == null,
                    ct
                );
            if (codeExists)
                return ServiceResult<LeaveTypeReadDto>.Fail(
                    $"Un type de congé avec le code '{dto.LeaveCode}' existe déjà."
                );
    ...
    public async Task<ServiceResult> DeleteLeaveTypeAsync(int id, int deletedBy, CancellationToken ct = default)
    {
        ...
        if (hasRequests)
            return ServiceResult.Fail(
                "Impossible de supprimer ce type de congé car il est utilisé dans des demandes de congés."
            );

        var hasBalances = await _db.LeaveBalances.AnyAsync(lb => lb.LeaveTypeId == id && lb.DeletedAt == null, ct);
        if (hasBalances)
            return ServiceResult.Fail(
                "Impossible de supprimer ce type de congé car il est utilisé dans des soldes de congés."
            );
    ...
    public async Task<ServiceResult<IEnumerable<LeaveTypePolicyReadDto>>> GetPoliciesAsync(
        int? companyId,
        int? leaveTypeId,
        CancellationToken ct = default
    )
    ...
    public async Task<ServiceResult<LeaveTypePolicyReadDto>> GetPolicyByIdAsync(int id, CancellationToken ct = default)
    {
        ...
        return p == null
            ? ServiceResult<LeaveTypePolicyReadDto>.Fail("Politique introuvable.")
            : ServiceResult<LeaveTypePolicyReadDto>.Ok(MapPolicy(p));
    }

    public async Task<ServiceResult<LeaveTypePolicyReadDto>> CreatePolicyAsync(
        LeaveTypePolicyCreateDto dto,
        int createdBy,
        CancellationToken ct = default
    )
    {
        ...
        var policyExists = await _db.LeaveTypePolicies.AnyAsync(
            p => p.CompanyId == dto.CompanyId && p.LeaveTypeId == dto.LeaveTypeId && p.DeletedAt == null,
            ct
        );
        if (policyExists)
            return ServiceResult<LeaveTypePolicyReadDto>.Fail(
                "Une politique existe déjà pour ce type de congé et cette entreprise."
            );
    ...
    public async Task<ServiceResult<LeaveTypePolicyReadDto>> PatchPolicyAsync(
        int id,
        LeaveTypePolicyPatchDto dto,
        int updatedBy,
        CancellationToken ct = default
    )
    {
        ...
        if (dto.MinConsecutiveDays != null)
        {
            if (dto.MinConsecutiveDays < 0)
                return ServiceResult<LeaveTypePolicyReadDto>.Fail(
                    "MinConsecutiveDays doit être supérieur ou égal à 0."
                );
    ...
    public async Task<ServiceResult<IEnumerable<LeaveTypeLegalRuleReadDto>>> GetLegalRulesAsync(
        int? leaveTypeId,
        CancellationToken ct = default
    )
    {
    ...
    public async Task<ServiceResult<LeaveTypeLegalRuleReadDto>> GetLegalRuleByIdAsync(
        int id,
        CancellationToken ct = default
    )
    {
        ...
        return r == null
            ? ServiceResult<LeaveTypeLegalRuleReadDto>.Fail("Règle légale introuvable.")
            : ServiceResult<LeaveTypeLegalRuleReadDto>.Ok(MapLegalRule(r));
    }

    public async Task<ServiceResult<LeaveTypeLegalRuleReadDto>> CreateLegalRuleAsync(
        LeaveTypeLegalRuleCreateDto dto,
        int createdBy,
        CancellationToken ct = default
    )
    {
        // Vérifier que le LeaveType existe
        var leaveType = await _db.LeaveTypes.FirstOrDefaultAsync(
            lt => lt.Id == dto.LeaveTypeId && lt.DeletedAt == null,
            ct
        );
        if (leaveType == null)
        ...
        var ruleExists = await _db.LeaveTypeLegalRules.AnyAsync(
            r => r.LeaveTypeId == dto.LeaveTypeId && r.EventCaseCode == dto.EventCaseCode,
            ct
        );
        if (ruleExists)
            return ServiceResult<LeaveTypeLegalRuleReadDto>.Fail(
                $"Une règle légale avec le code '{dto.EventCaseCode}' existe déjà pour ce type de congé."
            );
    ...
    public async Task<ServiceResult<LeaveTypeLegalRuleReadDto>> PatchLegalRuleAsync(
        int id,
        LeaveTypeLegalRulePatchDto dto,
        int updatedBy,
        CancellationToken ct = default
    )
    {
        var r = await _db.LeaveTypeLegalRules.Include(x => x.LeaveType).FirstOrDefaultAsync(x => x.Id == id, ct);
        if (r == null)
        ...
        {
            var codeExists = await _db.LeaveTypeLegalRules.AnyAsync(
                x => x.Id != id && x.LeaveTypeId == r.LeaveTypeId && x.EventCaseCode == dto.EventCaseCode,
                ct
            );
            if (codeExists)
                return ServiceResult<LeaveTypeLegalRuleReadDto>.Fail(
    ...
    public async Task<ServiceResult> DeleteLegalRuleAsync(int id, int deletedBy, CancellationToken ct = default)
    {
        ...
        if (hasRequests)
            return ServiceResult.Fail(
                "Impossible de supprimer cette règle car elle est utilisée dans des demandes de congés."
            );
    ...
    public async Task<ServiceResult<IEnumerable<LeaveRequestReadDto>>> GetLeaveRequestsByEmployeeIdAsync(
        int employeeId,
        CancellationToken ct = default
    )
    {
    ...
    public async Task<ServiceResult<LeaveRequestReadDto>> GetLeaveRequestByIdAsync(
        int id,
        CancellationToken ct = default
    )
    {
    ...
    public async Task<ServiceResult<IEnumerable<LeaveRequestReadDto>>> GetPendingApprovalAsync(
        int? companyId,
        CancellationToken ct = default
    )
    {
    ...
    public async Task<ServiceResult<LeaveRequestReadDto>> CreateLeaveRequestForSelfAsync(
    ...
    )
    {
        ...
        if (policy == null)
            return ServiceResult<LeaveRequestReadDto>.Fail(
                "Aucune politique de congé définie pour ce type de congé dans l'entreprise."
            );
    ...
    public async Task<ServiceResult> CreateLeaveRequestForOtherEmployeeAsync(
    ...
    )
    {
        ...
        if (!isRhOrAdmin)
        {
            var isManager = await _db
                .Employees.AsNoTracking()
                .AnyAsync(e => e.Id == targetEmployeeId && e.ManagerId == actorUserId, ct);
            if (!isManager)
                return ServiceResult.Fail(
                    "Vous devez être RH, Admin ou Manager pour créer un congé pour cet utilisateur."
                );
        }
    ...
    public async Task<ServiceResult<LeaveRequestReadDto>> CreateLeaveRequestAsync(
        int employeeId,
        LeaveRequestCreateDto dto,
        int createdBy,
        CancellationToken ct = default
    ...
    public Task<ServiceResult<LeaveRequestReadDto>> PatchLeaveRequestAsync(
        int id,
        LeaveRequestPatchDto dto,
        int updatedBy,
        CancellationToken ct = default
    ) => UpdateLeaveRequestDraftAsync(id, dto, updatedBy, ct);

    public Task<ServiceResult<LeaveRequestReadDto>> PutLeaveRequestAsync(
        int id,
        LeaveRequestPatchDto dto,
        int updatedBy,
        CancellationToken ct = default
    ...
    public async Task<ServiceResult<LeaveRequestReadDto>> SubmitLeaveRequestAsync(
        int id,
        int userId,
        CancellationToken ct = default
    )
    {
        ...
        {
            var policy = await ResolvePolicyForBalanceCheckAsync(
                request.CompanyId,
                request.LeaveTypeId,
                request.StartDate,
                ct
            ...
            if (policy == null)
                return ServiceResult<LeaveRequestReadDto>.Fail(
                    "Aucune politique active trouvée pour ce type de congé."
                );
            if (policy.RequiresBalance && !policy.AllowNegativeBalance)
            {
                var total = await GetTotalNonExpiredClosingDaysAsync(
                    request.CompanyId,
                    request.EmployeeId,
                    request.LeaveTypeId,
                    request.StartDate,
        ...
        if (hasOverlap)
            return ServiceResult<LeaveRequestReadDto>.Fail(
                "Cette période chevauche une demande déjà soumise ou approuvée."
            );
        ...
        _db.LeaveRequestApprovalHistories.Add(
            new LeaveRequestApprovalHistory
            {
    ...
    public async Task<ServiceResult<LeaveRequestReadDto>> ApproveLeaveRequestAsync(
        int id,
        string? comment,
        int decidedBy,
        CancellationToken ct = default
    ...
    {
        var request = await _db
            .LeaveRequests.Include(lr => lr.Policy)
            .Include(lr => lr.LegalRule)
            .Include(lr => lr.LeaveType)
            .FirstOrDefaultAsync(lr => lr.Id == id && lr.DeletedAt == null, ct);
    ...
    public async Task<ServiceResult<LeaveRequestReadDto>> RejectLeaveRequestAsync(
        int id,
        string? comment,
        int decidedBy,
        CancellationToken ct = default
    ...
    public async Task<ServiceResult<LeaveRequestReadDto>> CancelLeaveRequestAsync(
        int id,
        string? comment,
        int userId,
        CancellationToken ct = default
    )
    {
        ...
        if (request.Status != LeaveRequestStatus.Approved && request.Status != LeaveRequestStatus.Submitted)
            return ServiceResult<LeaveRequestReadDto>.Fail(
                "Seules les demandes approuvées ou soumises peuvent être annulées."
            );
    ...
    public async Task<ServiceResult<LeaveRequestReadDto>> RenounceLeaveRequestAsync(
        int id,
        int userId,
        CancellationToken ct = default
    )
    {
        ...
        if (request.Status != LeaveRequestStatus.Approved)
            return ServiceResult<LeaveRequestReadDto>.Fail(
                "Seules les demandes approuvées peuvent faire l'objet d'une renonciation."
            );
    ...
    private async Task<Payzen.Domain.Entities.Employee.Employee?> ResolveEmployeeForSelfAsync(
        int userId,
    ...
    private async Task<LeaveTypePolicy?> ResolveCreatePolicyAsync(
        int companyId,
        LeaveRequestCreateDto dto,
        CancellationToken ct
    )
    ...
    private async Task<LeaveTypePolicy?> ResolvePolicyForBalanceCheckAsync(
        int companyId,
        int leaveTypeId,
        DateOnly asOfDate,
        CancellationToken ct
    ...
    )
    {
        ...
        var workingDays = await _workingDays.CalculateWorkingDaysAsync(
            employee.CompanyId,
            dto.StartDate,
            dto.EndDate,
            ct
    ...
    private async Task<ServiceResult<LeaveRequestReadDto>> UpdateLeaveRequestDraftAsync(
        int id,
        LeaveRequestPatchDto dto,
        int userId,
        CancellationToken ct
    )
    {
        ...
        request.WorkingDaysDeducted = await _workingDays.CalculateWorkingDaysAsync(
            request.CompanyId,
            request.StartDate,
            request.EndDate,
            ct
    ...
    public Task<ServiceResult<IEnumerable<LeaveBalanceReadDto>>> GetBalancesAsync(
        int employeeId,
        int? leaveTypeId,
        CancellationToken ct = default
    ...
    )
    {
        ...
        var raw = await q.OrderBy(b => b.Year)
            .ThenBy(b => b.Month)
    ...
    public async Task<ServiceResult<IEnumerable<LeaveBalanceReadDto>>> GetBalancesByYearAsync(
        int employeeId,
        int year,
        CancellationToken ct = default
    )
    ...
    public async Task<ServiceResult<IEnumerable<LeaveBalanceReadDto>>> GetBalancesByYearMonthAsync(
        int employeeId,
        int year,
        int month,
        CancellationToken ct = default
    ...
    public async Task<ServiceResult<object>> GetBalanceSummaryAsync(
        int employeeId,
        int? companyId,
        CancellationToken ct = default
    )
    {
        ...
        var q = _db.LeaveBalances.Where(lb => lb.EmployeeId == employeeId && lb.DeletedAt == null);
        if (companyId.HasValue)
            q = q.Where(lb => lb.CompanyId == companyId.Value);
    ...
    )
    {
        ...
        var balances = await _db
            .LeaveBalances.Where(b =>
                b.EmployeeId == employeeId && b.Year == year && b.Month == month && b.DeletedAt == null
            )
            .ToListAsync(ct);
        ...
        if (balances.Count == 0)
        {
            ...
            var leaveTypesQuery = _db.LeaveTypes.AsNoTracking().Where(lt => lt.DeletedAt == null && lt.IsActive);

            if (leaveTypeId.HasValue)
                leaveTypesQuery = leaveTypesQuery.Where(lt => lt.Id == leaveTypeId.Value);
            else
                leaveTypesQuery = leaveTypesQuery.Where(lt =>
                    lt.CompanyId == null || lt.CompanyId == effectiveCompanyId
                );
            ...
            var seedBalances = leaveTypes
                .Select(lt => new LeaveBalance
                {
                    EmployeeId = employeeId,
        ...
        if (recalculatedCount > 0)
        {
            await _leaveEventLog.LogEventAsync(
                ...
                "BalanceRecalculated",
                null,
                $"{recalculatedCount} solde(s) recalculé(s) pour {year}-{month:D2}",
                userId,
                ct
            );
        }
    ...
    public async Task<ServiceResult<LeaveBalanceReadDto>> CreateBalanceAsync(
        LeaveBalanceCreateDto dto,
        int createdBy,
        CancellationToken ct = default
    )
    {
        ...
        var balanceExists = await _db.LeaveBalances.AnyAsync(
            b =>
        ...
        if (balanceExists)
            return ServiceResult<LeaveBalanceReadDto>.Fail(
                "Un solde existe déjà pour cet employé, ce type de congé et ce mois."
            );
    ...
    public async Task<ServiceResult<LeaveBalanceReadDto>> PatchBalanceAsync(
        int id,
        LeaveBalancePatchDto dto,
        int updatedBy,
        CancellationToken ct = default
    )
    {
    ...
    public async Task<ServiceResult<IEnumerable<LeaveCarryOverAgreementReadDto>>> GetCarryOverAgreementsAsync(
        int employeeId,
        CancellationToken ct = default
    )
    {
        var list = await _db
            .LeaveCarryOverAgreements.Where(a => a.EmployeeId == employeeId && a.DeletedAt == null)
            .Select(a => MapCarryOver(a))
            .ToListAsync(ct);
        return ServiceResult<IEnumerable<LeaveCarryOverAgreementReadDto>>.Ok(list);
    }

    public async Task<ServiceResult<LeaveCarryOverAgreementReadDto>> GetCarryOverByIdAsync(
        int id,
        CancellationToken ct = default
    )
    {
    ...
    public async Task<ServiceResult<LeaveCarryOverAgreementReadDto>> CreateCarryOverAgreementAsync(
        LeaveCarryOverAgreementCreateDto dto,
        int createdBy,
        CancellationToken ct = default
    )
    {
        ...
        var leaveType = await _db
            .LeaveTypes.Include(lt => lt.Policies)
            .FirstOrDefaultAsync(lt => lt.Id == dto.LeaveTypeId && lt.DeletedAt == null, ct);
        if (leaveType == null)
        ...
        var policy = await _db.LeaveTypePolicies.FirstOrDefaultAsync(
            p => p.LeaveTypeId == dto.LeaveTypeId && p.CompanyId == dto.CompanyId && p.DeletedAt == null,
            ct
        );
        if (policy != null && !policy.AllowCarryover)
            return ServiceResult<LeaveCarryOverAgreementReadDto>.Fail(
                "Le type de congé n'autorise pas le report de congés."
            );
        if (policy != null)
        {
            ...
            if (yearsDiff > policy.MaxCarryoverYears)
                return ServiceResult<LeaveCarryOverAgreementReadDto>.Fail(
                    $"Le report ne peut pas dépasser {policy.MaxCarryoverYears} année(s)."
                );
        }
    ...
    public async Task<ServiceResult<LeaveCarryOverAgreementReadDto>> PatchCarryOverAgreementAsync(
        int id,
        LeaveCarryOverAgreementPatchDto dto,
        int updatedBy,
        CancellationToken ct = default
    )
    {
    ...
    public async Task<ServiceResult> DeleteCarryOverAgreementAsync(
        int id,
        int deletedBy,
        CancellationToken ct = default
    )
    ...
    public async Task<ServiceResult<IEnumerable<LeaveRequestAttachmentReadDto>>> GetAttachmentsAsync(
        int leaveRequestId,
        CancellationToken ct = default
    )
    {
        var list = await _db
            .LeaveRequestAttachments.Where(a => a.LeaveRequestId == leaveRequestId)
            .OrderByDescending(a => a.CreatedAt)
            .Select(a => MapAttachment(a))
            .ToListAsync(ct);
        return ServiceResult<IEnumerable<LeaveRequestAttachmentReadDto>>.Ok(list);
    }

    public async Task<ServiceResult<LeaveRequestAttachmentReadDto>> GetAttachmentByIdAsync(
        int id,
        CancellationToken ct = default
    )
    {
    ...
    public async Task<ServiceResult<LeaveRequestAttachmentReadDto>> CreateAttachmentAsync(
        LeaveRequestAttachmentCreateDto dto,
        int createdBy,
        CancellationToken ct = default
    )
    {
        // Vérifier que la LeaveRequest existe
        var leaveRequest = await _db.LeaveRequests.FirstOrDefaultAsync(
            lr => lr.Id == dto.LeaveRequestId && lr.DeletedAt == null,
            ct
        );
    ...
    public async Task<ServiceResult<(byte[] content, string fileName, string contentType)>> GetAttachmentDownloadAsync(
        int attachmentId,
        CancellationToken ct = default
    )
    {
        ...
        if (string.IsNullOrEmpty(a.FilePath) || !System.IO.File.Exists(a.FilePath))
            return ServiceResult<(byte[], string, string)>.Ok(
                (Array.Empty<byte>(), a.FileName ?? "attachment", a.FileType ?? "application/octet-stream")
            );
        var bytes = await System.IO.File.ReadAllBytesAsync(a.FilePath, ct);
        return ServiceResult<(byte[], string, string)>.Ok(
            (bytes, a.FileName ?? "attachment", a.FileType ?? "application/octet-stream")
        );
    }
    ...
    public async Task<ServiceResult<IEnumerable<LeaveRequestExemptionReadDto>>> GetExemptionsAsync(
        int leaveRequestId,
        CancellationToken ct = default
    )
    {
        var list = await _db
            .LeaveRequestExemptions.Where(e => e.LeaveRequestId == leaveRequestId && e.DeletedAt == null)
            .OrderBy(e => e.ExemptionDate)
            .Select(e => MapExemption(e))
            .ToListAsync(ct);
        return ServiceResult<IEnumerable<LeaveRequestExemptionReadDto>>.Ok(list);
    }

    public async Task<ServiceResult<LeaveRequestExemptionReadDto>> GetExemptionByIdAsync(
        int id,
        CancellationToken ct = default
    )
    {
    ...
    public async Task<ServiceResult<LeaveRequestExemptionReadDto>> CreateExemptionAsync(
        LeaveRequestExemptionCreateDto dto,
        int createdBy,
        CancellationToken ct = default
    )
    {
        ...
        if (dto.ExemptionDate < leaveRequest.StartDate || dto.ExemptionDate > leaveRequest.EndDate)
            return ServiceResult<LeaveRequestExemptionReadDto>.Fail(
                "La date d'exemption doit être comprise entre les dates de début et de fin de la demande."
            );
        ...
        if (dto.ReasonType == LeaveExemptionReasonType.EmployeeAbsence && !dto.EmployeeAbsenceId.HasValue)
            return ServiceResult<LeaveRequestExemptionReadDto>.Fail(
                "EmployeeAbsenceId est obligatoire pour le type EmployeeAbsence."
            );

        // Unicité par date
        var exemptionExists = await _db.LeaveRequestExemptions.AnyAsync(
            e => e.LeaveRequestId == dto.LeaveRequestId && e.ExemptionDate == dto.ExemptionDate && e.DeletedAt == null,
            ct
        );
        if (exemptionExists)
    ...
    public async Task<ServiceResult<LeaveRequestExemptionReadDto>> PatchExemptionAsync(
        int id,
        LeaveRequestExemptionPatchDto dto,
        int updatedBy,
        CancellationToken ct = default
    )
    {
        var e = await _db
            .LeaveRequestExemptions.Include(x => x.LeaveRequest)
            .FirstOrDefaultAsync(x => x.Id == id && x.DeletedAt == null, ct);
        if (e == null)
            return ServiceResult<LeaveRequestExemptionReadDto>.Fail("Exemption introuvable.");
        ...
        if (dto.ExemptionDate != null)
        {
            if (dto.ExemptionDate < e.LeaveRequest.StartDate || dto.ExemptionDate > e.LeaveRequest.EndDate)
                return ServiceResult<LeaveRequestExemptionReadDto>.Fail(
                    "La date d'exemption doit être comprise entre les dates de la demande."
                );
    ...
    )
    {
        ...
        var list = await q.OrderByDescending(l => l.CreatedAt).Take(1000).Select(l => MapAuditLog(l)).ToListAsync(ct);
    ...
    public async Task<ServiceResult<IEnumerable<LeaveAuditLogReadDto>>> GetAuditLogsByEmployeeAsync(
        int employeeId,
        CancellationToken ct = default
    )
    {
        var list = await _db
            .LeaveAuditLogs.Where(l => l.EmployeeId == employeeId)
    ...
    public async Task<ServiceResult<LeaveAuditLogReadDto>> CreateAuditLogAsync(
        LeaveAuditLogCreateDto dto,
        CancellationToken ct = default
    )
    {
    ...
    private static LeaveTypeReadDto MapLeaveType(LeaveType lt) =>
        new()
        {
            Id = lt.Id,
    ...
    private static LeaveTypePolicyReadDto MapPolicy(LeaveTypePolicy p) =>
        new()
        {
            Id = p.Id,
    ...
    private static LeaveTypeLegalRuleReadDto MapLegalRule(LeaveTypeLegalRule r) =>
        new()
        {
            Id = r.Id,
    ...
    private static LeaveRequestReadDto MapLeaveRequest(LeaveRequest lr) =>
        new()
        {
            Id = lr.Id,
    ...
    private static LeaveCarryOverAgreementReadDto MapCarryOver(LeaveCarryOverAgreement a) =>
        new()
        {
            Id = a.Id,
    ...
    private static LeaveRequestAttachmentReadDto MapAttachment(LeaveRequestAttachment a) =>
        new()
        {
            Id = a.Id,
    ...
    private static LeaveRequestExemptionReadDto MapExemption(LeaveRequestExemption e) =>
        new()
        {
            Id = e.Id,
    ...
    private static LeaveAuditLogReadDto MapAuditLog(LeaveAuditLog l) =>
        new()
        {
            Id = l.Id,
...
public class LeaveTypeService : ILeaveTypeService
{
    ...
    public Task<ServiceResult<IEnumerable<LeaveTypeReadDto>>> GetAllAsync(
        int? companyId,
        CancellationToken ct = default
    ) => _svc.GetLeaveTypesAsync(companyId, ct);

    public Task<ServiceResult<LeaveTypeReadDto>> GetByIdAsync(int id, CancellationToken ct = default) =>
        _svc.GetLeaveTypeByIdAsync(id, ct);

    public Task<ServiceResult<LeaveTypeReadDto>> CreateAsync(
        LeaveTypeCreateDto dto,
        int createdBy,
        CancellationToken ct = default
    ) => _svc.CreateLeaveTypeAsync(dto, createdBy, ct);

    public Task<ServiceResult<LeaveTypeReadDto>> PatchAsync(
        int id,
        LeaveTypePatchDto dto,
        int updatedBy,
        CancellationToken ct = default
    ) => _svc.PatchLeaveTypeAsync(id, dto, updatedBy, ct);

    public Task<ServiceResult> DeleteAsync(int id, int deletedBy, CancellationToken ct = default) =>
        _svc.DeleteLeaveTypeAsync(id, deletedBy, ct);

    public Task<ServiceResult<IEnumerable<LeaveTypePolicyReadDto>>> GetPoliciesAsync(
        int? companyId,
        int? leaveTypeId,
        CancellationToken ct = default
    ) => _svc.GetPoliciesAsync(companyId, leaveTypeId, ct);

    public Task<ServiceResult<LeaveTypePolicyReadDto>> CreatePolicyAsync(
        LeaveTypePolicyCreateDto dto,
        int createdBy,
        CancellationToken ct = default
    ) => _svc.CreatePolicyAsync(dto, createdBy, ct);

    public Task<ServiceResult<LeaveTypePolicyReadDto>> PatchPolicyAsync(
        int id,
        LeaveTypePolicyPatchDto dto,
        int updatedBy,
        CancellationToken ct = default
    ) => _svc.PatchPolicyAsync(id, dto, updatedBy, ct);

    public Task<ServiceResult> DeletePolicyAsync(int id, int deletedBy, CancellationToken ct = default) =>
        _svc.DeletePolicyAsync(id, deletedBy, ct);

    public Task<ServiceResult<IEnumerable<LeaveTypeLegalRuleReadDto>>> GetLegalRulesAsync(
        int? leaveTypeId,
        CancellationToken ct = default
    ) => _svc.GetLegalRulesAsync(leaveTypeId, ct);

    public Task<ServiceResult<LeaveTypeLegalRuleReadDto>> CreateLegalRuleAsync(
        LeaveTypeLegalRuleCreateDto dto,
        int createdBy,
        CancellationToken ct = default
    ) => _svc.CreateLegalRuleAsync(dto, createdBy, ct);

    public Task<ServiceResult> DeleteLegalRuleAsync(int id, int deletedBy, CancellationToken ct = default) =>
        _svc.DeleteLegalRuleAsync(id, deletedBy, ct);
}
...
public class LeaveBalanceService : ILeaveBalanceService
{
    ...
    public Task<ServiceResult<IEnumerable<LeaveBalanceReadDto>>> GetByEmployeeAsync(
        int employeeId,
        int? leaveTypeId,
        CancellationToken ct = default
    ) => _svc.GetBalancesAsync(employeeId, leaveTypeId, ct);

    public Task<ServiceResult<LeaveBalanceReadDto>> CreateAsync(
        LeaveBalanceCreateDto dto,
        int createdBy,
        CancellationToken ct = default
    ) => _svc.CreateBalanceAsync(dto, createdBy, ct);

    public Task<ServiceResult<LeaveBalanceReadDto>> PatchAsync(
        int id,
        LeaveBalancePatchDto dto,
        int updatedBy,
        CancellationToken ct = default
    ) => _svc.PatchBalanceAsync(id, dto, updatedBy, ct);

    public Task<ServiceResult<IEnumerable<LeaveCarryOverAgreementReadDto>>> GetCarryOverAgreementsAsync(
        int employeeId,
        CancellationToken ct = default
    ) => _svc.GetCarryOverAgreementsAsync(employeeId, ct);

    public Task<ServiceResult<LeaveCarryOverAgreementReadDto>> CreateCarryOverAgreementAsync(
        LeaveCarryOverAgreementCreateDto dto,
        int createdBy,
        CancellationToken ct = default
    ) => _svc.CreateCarryOverAgreementAsync(dto, createdBy, ct);

    public Task<ServiceResult> DeleteCarryOverAgreementAsync(int id, int deletedBy, CancellationToken ct = default) =>
        _svc.DeleteCarryOverAgreementAsync(id, deletedBy, ct);
}
...
public class LeaveAuditLogService : ILeaveAuditLogService
{
    ...
    public Task<ServiceResult<IEnumerable<LeaveAuditLogReadDto>>> GetAsync(
        int? companyId,
        int? leaveRequestId,
        CancellationToken ct = default
    ) => _svc.GetAuditLogsAsync(companyId, leaveRequestId, ct);

    public Task<ServiceResult<LeaveAuditLogReadDto>> CreateAsync(
        LeaveAuditLogCreateDto dto,
        CancellationToken ct = default
    ) => _svc.CreateAuditLogAsync(dto, ct);
}
