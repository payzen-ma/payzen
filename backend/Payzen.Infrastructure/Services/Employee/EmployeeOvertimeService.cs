...
public partial class EmployeeOvertimeService : IEmployeeOvertimeService
{
    ...
    public async Task<ServiceResult<IEnumerable<EmployeeOvertimeListDto>>> GetAllAsync(
        int? companyId, int? employeeId, OvertimeStatus? status, DateOnly? fromDate, DateOnly? toDate,
        bool? isProcessedInPayroll, CancellationToken ct = default)
    {
    ...
    public async Task<ServiceResult<EmployeeOvertimeStatsDto>> GetStatsAsync(int companyId, int? employeeId, CancellationToken ct = default)
    {
        var q = _db.EmployeeOvertimes.AsNoTracking()
            .Where(o => o.DeletedAt == null && o.Employee.CompanyId == companyId);
        if (employeeId.HasValue)
            q = q.Where(o => o.EmployeeId == employeeId.Value);

        var totalHours = await q.SumAsync(o => (decimal?)o.DurationInHours, ct) ?? 0m;
        var pending = await q.CountAsync(o => o.Status == OvertimeStatus.Draft || o.Status == OvertimeStatus.Submitted, ct);
        ...
        return ServiceResult<EmployeeOvertimeStatsDto>.Ok(new EmployeeOvertimeStatsDto
        {
    ...
    public async Task<ServiceResult<IEnumerable<EmployeeOvertimeListDto>>> GetByEmployeeAsync(int employeeId, CancellationToken ct = default)
    {
        var rows = await _db.EmployeeOvertimes.AsNoTracking()
    ...
    public Task<ServiceResult<EmployeeOvertimeCreateOutcomeDto>> CreateAsync(EmployeeOvertimeCreateDto dto, int createdBy, CancellationToken ct = default)
        => CreateOvertimesFullAsync(dto, createdBy, ct);

    public async Task<ServiceResult<EmployeeOvertimeReadDto>> UpdateAsync(int id, EmployeeOvertimeUpdateDto dto, int updatedBy, CancellationToken ct = default)
    {
        ...
        if (!o.CanBeModified())
            return ServiceResult<EmployeeOvertimeReadDto>.Fail("Seuls les overtimes en brouillon peuvent être modifiés");
    ...
    public async Task<ServiceResult<EmployeeOvertimeReadDto>> SubmitAsync(int id, EmployeeOvertimeSubmitDto dto, int userId, CancellationToken ct = default)
    {
        var o = await _db.EmployeeOvertimes.FirstOrDefaultAsync(x => x.Id == id && x.DeletedAt == null, ct);
        if (o == null)
            return ServiceResult<EmployeeOvertimeReadDto>.Fail("Overtime non trouvé");
        ...
        var isRh = user?.UsersRoles?.Any(ur =>
            ur.Role != null && ur.Role.Name.Equals("RH", StringComparison.OrdinalIgnoreCase)) ?? false;
    ...
    public async Task<ServiceResult<EmployeeOvertimeReadDto>> DecideAsync(int id, EmployeeOvertimeApprovalDto dto, int decidedBy, CancellationToken ct = default)
    {
    ...
    public async Task<ServiceResult<EmployeeOvertimeReadDto>> CancelAsync(int id, int userId, CancellationToken ct = default)
    {
        ...
        if (overtime.Status != OvertimeStatus.Submitted && overtime.Status != OvertimeStatus.Approved)
            return ServiceResult<EmployeeOvertimeReadDto>.Fail("Seuls les overtimes soumis ou approuvés peuvent être annulés");
    ...
    {
        var o = await _db.EmployeeOvertimes.AsNoTracking()
            .Include(x => x.Employee)
            .Include(x => x.Holiday)
            .FirstOrDefaultAsync(x => x.Id == id && x.DeletedAt == null, ct);
    ...
    private static EmployeeOvertimeReadDto MapToReadDto(EmployeeOvertime o, string? approvedByName) => new()
    {
        Id = o.Id,
