...
public class CompanyService : ICompanyService
{
    ...
    {
        var c = await _db.Companies.Include(c => c.City).Include(c => c.Country)
            .FirstOrDefaultAsync(c => c.Id == id && c.DeletedAt == null, ct);
        return c == null ? ServiceResult<CompanyReadDto>.Fail("Entreprise non trouvée") : ServiceResult<CompanyReadDto>.Ok(MapToRead(c));
    }

    public async Task<ServiceResult<IEnumerable<CompanyListDto>>> GetByCityAsync(int cityId, CancellationToken ct = default)
    {
        ...
        var list = await _db.Companies.Include(c => c.City).Include(c => c.Country)
    ...
    public async Task<ServiceResult<IEnumerable<CompanyListDto>>> GetByCountryAsync(int countryId, CancellationToken ct = default)
    {
    ...
    public async Task<ServiceResult<IEnumerable<CompanyListDto>>> SearchAsync(string searchTerm, CancellationToken ct = default)
    {
        var q = searchTerm.Trim().ToLower();
        var list = await _db.Companies.Include(c => c.City).Include(c => c.Country)
            .Where(c => c.DeletedAt == null && (c.CompanyName.ToLower().Contains(q) || c.Email.ToLower().Contains(q) || (c.CnssNumber != null && c.CnssNumber.ToLower().Contains(q))))
            .OrderBy(c => c.CompanyName)
            .Select(c => MapToList(c)).ToListAsync(ct);
        return ServiceResult<IEnumerable<CompanyListDto>>.Ok(list);
    }

    public async Task<ServiceResult<IEnumerable<CompanyListDto>>> GetManagedByAsync(int expertCompanyId, CancellationToken ct = default)
    {
        var managing = await _db.Companies.AsNoTracking().FirstOrDefaultAsync(c => c.Id == expertCompanyId && c.DeletedAt == null, ct);
        if (managing == null)
    ...
    {
        var countries = await _db.Countries.Where(c => c.DeletedAt == null).OrderBy(c => c.CountryName)
            .Select(c => new CountryFormDto { Id = c.Id, CountryName = c.CountryName, CountryNameAr = c.CountryNameAr, CountryCode = c.CountryCode, CountryPhoneCode = c.CountryPhoneCode }).ToListAsync(ct);
        var cities = await _db.Cities.Where(c => c.DeletedAt == null).Include(c => c.Country)
            .OrderBy(c => c.Country != null ? c.Country.CountryName : "").ThenBy(c => c.CityName)
            .Select(c => new CityFormDto { Id = c.Id, CityName = c.CityName, CountryId = c.CountryId, CountryName = c.Country != null ? c.Country.CountryName : null }).ToListAsync(ct);
    ...
    public async Task<ServiceResult<CompanyCreateResponseDto>> CreateAsync(CompanyCreateDto dto, int createdBy, CancellationToken ct = default)
    {
        ...
        var country = await _db.Countries.AsNoTracking().FirstOrDefaultAsync(c => c.Id == dto.CountryId && c.DeletedAt == null, ct);
        if (country == null)
        ...
        else
        {
            ...
            var duplicateCity = await _db.Cities
                .FirstOrDefaultAsync(c => c.CountryId == dto.CountryId && c.CityName.ToLower() == dto.CityName!.ToLower() && c.DeletedAt == null, ct);
        ...
        try
        {
            ...
            var activeStatus = await _db.Statuses.FirstOrDefaultAsync(s => s.Code.ToLower() == "active" && s.DeletedAt == null, ct);
    ...
    public async Task<ServiceResult<CompanyCreateResponseDto>> CreateByExpertAsync(CompanyCreateByExpertDto dto, int createdBy, CancellationToken ct = default)
    {
        var managing = await _db.Companies.AsNoTracking().FirstOrDefaultAsync(c => c.Id == dto.ManagedByCompanyId && c.DeletedAt == null, ct);
        if (managing == null)
            return ServiceResult<CompanyCreateResponseDto>.Fail("Cabinet expert gestionnaire introuvable");
        if (!managing.IsCabinetExpert)
            return ServiceResult<CompanyCreateResponseDto>.Fail("La société spécifiée comme gestionnaire n'est pas un cabinet expert");

        var currentUser = await _db.Users.Include(u => u.Employee).FirstOrDefaultAsync(u => u.Id == createdBy && u.DeletedAt == null, ct);
    ...
    public async Task<ServiceResult<CompanyReadDto>> PatchAsync(int id, CompanyUpdateDto dto, int updatedBy, CancellationToken ct = default)
    {
        if (dto == null)
            return ServiceResult<CompanyReadDto>.Fail("Données de mise à jour requises");

        var c = await _db.Companies.Include(x => x.City).Include(x => x.Country)
        ...
        if (!string.IsNullOrWhiteSpace(dto.CompanyName) && dto.CompanyName.Trim() != c.CompanyName)
        {
            ...
            if (await _db.Companies.AnyAsync(x => x.CompanyName.ToLower() == name.ToLower() && x.DeletedAt == null && x.Id != id, ct))
                return ServiceResult<CompanyReadDto>.Fail("Une autre entreprise utilise déjà ce nom");
            await _companyEventLog.LogEventAsync(c.Id, "CompanyName_Changed", c.CompanyName, null, name, null, updatedBy, ct);
            c.CompanyName = name;
        }
        ...
        if (!string.IsNullOrWhiteSpace(dto.PhoneNumber) && dto.PhoneNumber.Trim() != c.PhoneNumber)
        {
            ...
            await _companyEventLog.LogEventAsync(c.Id, "Phone_Changed", c.PhoneNumber, null, phone, null, updatedBy, ct);
            c.PhoneNumber = phone;
        }

        if (!string.IsNullOrWhiteSpace(dto.CompanyAddress) && dto.CompanyAddress.Trim() != c.CompanyAddress)
        {
            await _companyEventLog.LogEventAsync(c.Id, "Address_Changed", c.CompanyAddress, null, dto.CompanyAddress.Trim(), null, updatedBy, ct);
            c.CompanyAddress = dto.CompanyAddress.Trim();
        }

        if (dto.IsCabinetExpert.HasValue && dto.IsCabinetExpert.Value != c.IsCabinetExpert)
        {
            await _companyEventLog.LogSimpleEventAsync(c.Id, "IsCabinetExpert_Changed", c.IsCabinetExpert.ToString(), dto.IsCabinetExpert.Value.ToString(), updatedBy, ct);
        ...
        if (dto.CountryId.HasValue || !string.IsNullOrWhiteSpace(dto.CountryName))
        {
            ...
            {
                var country = await _db.Countries.AsNoTracking().FirstOrDefaultAsync(x => x.Id == dto.CountryId.Value && x.DeletedAt == null, ct);
                if (country == null)
                    return ServiceResult<CompanyReadDto>.Fail("Pays non trouvé");
                await _companyEventLog.LogEventAsync(c.Id, "Country_Changed", c.CountryId.ToString(), c.CountryId, country.CountryName, country.Id, updatedBy, ct);
            ...
            else if (!string.IsNullOrWhiteSpace(dto.CountryName))
            {
                ...
                var country = await _db.Countries.FirstOrDefaultAsync(x => x.CountryName.ToLower() == name.ToLower() && x.DeletedAt == null, ct);
                if (country != null)
                {
                ...
                else
                {
                    ...
                    await _db.SaveChangesAsync(ct);
                    await _companyEventLog.LogEventAsync(c.Id, "Country_Changed", c.CountryId.ToString(), c.CountryId, newCountry.CountryName, newCountry.Id, updatedBy, ct);
        ...
        if (dto.CityId.HasValue || !string.IsNullOrWhiteSpace(dto.CityName))
        {
            ...
            if (dto.CityId.HasValue)
            {
                ...
                else
                {
                    ...
                    if (city.CountryId != c.CountryId)
                        return ServiceResult<CompanyReadDto>.Fail("La ville choisie n'appartient pas au pays courant de l'entreprise");
                    await _companyEventLog.LogEventAsync(c.Id, "City_Changed", c.CityId.ToString(), c.CityId, city.CityName, city.Id, updatedBy, ct);
                    c.CityId = city.Id;
                }
            }
            else
            {
                ...
                var city = await _db.Cities.FirstOrDefaultAsync(x => x.CityName.ToLower() == cityName.ToLower() && x.CountryId == c.CountryId && x.DeletedAt == null, ct);
                if (city != null)
                {
                    if (c.CityId != city.Id)
                    {
                ...
                {
                    var newCity = new City { CityName = cityName, CountryId = c.CountryId, CreatedBy = updatedBy };
                    _db.Cities.Add(newCity);
                    await _db.SaveChangesAsync(ct);
                    await _companyEventLog.LogEventAsync(c.Id, "City_Changed", c.CityId.ToString(), c.CityId, newCity.CityName, newCity.Id, updatedBy, ct);
                    c.CityId = newCity.Id;
                }
            }
        }
        ...
        if (!string.IsNullOrWhiteSpace(dto.CnssNumber))
        {
            ...
            {
                await _companyEventLog.LogEventAsync(c.Id, "Cnss_Changed", c.CnssNumber, null, cnss, null, updatedBy, ct);
                c.CnssNumber = cnss;
            }
        }
        ...
        {
            await _companyEventLog.LogEventAsync(c.Id, "Rib_Changed", c.RibNumber, null, dto.RibNumber.Trim(), null, updatedBy, ct);
            c.RibNumber = dto.RibNumber.Trim();
        }

        if (!string.IsNullOrWhiteSpace(dto.PatenteNumber) && dto.PatenteNumber.Trim() != c.PatenteNumber)
        {
            await _companyEventLog.LogEventAsync(c.Id, "Patent_Changed", c.PatenteNumber, null, dto.PatenteNumber.Trim(), null, updatedBy, ct);
        ...
        {
            await _companyEventLog.LogEventAsync(c.Id, "FoundingDate_Changed", c.FoundingDate?.ToString("o"), null, dto.FoundingDate.Value.ToString("o"), null, updatedBy, ct);
            c.FoundingDate = dto.FoundingDate.Value;
        }

        if (dto.IsActive.HasValue && dto.IsActive.Value != c.isActive)
        {
            await _companyEventLog.LogSimpleEventAsync(c.Id, "Status_Changed", c.isActive ? "Active" : "Inactive", dto.IsActive.Value ? "Active" : "Inactive", updatedBy, ct);
    ...
    public async Task<ServiceResult<IEnumerable<CompanyHistoryDto>>> GetHistoryAsync(int companyId, CancellationToken ct = default)
    {
        ...
        var events = await _db.CompanyEventLogs
            ...
            .Select(l => new { l.eventName, l.oldValue, l.oldValueId, l.newValue, l.newValueId, l.CreatedAt, l.CreatedBy, l.employeeId })
            .ToListAsync(ct);
        ...
        var users = await _db.Users.AsNoTracking().Include(u => u.Employee)
            .Where(u => creatorIds.Contains(u.Id)).ToListAsync(ct);
        var usersRoles = await _db.UsersRoles.AsNoTracking().Include(ur => ur.Role)
            .Where(ur => creatorIds.Contains(ur.UserId) && ur.DeletedAt == null).ToListAsync(ct);
        var roleByUser = usersRoles.GroupBy(ur => ur.UserId).ToDictionary(g => g.Key, g => string.Join(", ", g.Select(x => x.Role?.Name).Where(n => n != null)));
    ...
    public async Task<ServiceResult<IEnumerable<DepartementReadDto>>> GetAllDepartementsAsync(CancellationToken ct = default)
    {
        var list = await _db.Departements
            .Where(d => d.DeletedAt == null)
            .Include(d => d.Company)
            .OrderBy(d => d.DepartementName)
            .Select(d => new DepartementReadDto { Id = d.Id, DepartementName = d.DepartementName, CompanyId = d.CompanyId, CompanyName = d.Company != null ? d.Company.CompanyName : "", CreatedAt = d.CreatedAt.DateTime })
            .ToListAsync(ct);
    ...
    {
        var d = await _db.Departements
            .Where(x => x.Id == id && x.DeletedAt == null)
            .Include(x => x.Company)
            .Select(x => new DepartementReadDto { Id = x.Id, DepartementName = x.DepartementName, CompanyId = x.CompanyId, CompanyName = x.Company != null ? x.Company.CompanyName : "", CreatedAt = x.CreatedAt.DateTime })
            .FirstOrDefaultAsync(ct);
        return d == null ? ServiceResult<DepartementReadDto>.Fail("Département non trouvé") : ServiceResult<DepartementReadDto>.Ok(d);
    }

    public async Task<ServiceResult<IEnumerable<DepartementReadDto>>> GetDepartementsAsync(int companyId, CancellationToken ct = default)
    {
    ...
    public async Task<ServiceResult<DepartementReadDto>> CreateDepartementAsync(DepartementCreateDto dto, int createdBy, CancellationToken ct = default)
    {
        ...
        var nameExists = await _db.Departements.AnyAsync(d => d.CompanyId == dto.CompanyId && d.DepartementName == dto.DepartementName && d.DeletedAt == null, ct);
        if (nameExists)
            return ServiceResult<DepartementReadDto>.Fail("Un département avec ce nom existe déjà dans cette société");

        var dep = new Departement { DepartementName = dto.DepartementName, CompanyId = dto.CompanyId, CreatedBy = createdBy };
        _db.Departements.Add(dep);
        await _db.SaveChangesAsync(ct);

        await _companyEventLog.LogEventAsync(dep.CompanyId, "Departement_Created", null, null, dep.DepartementName, dep.Id, createdBy, ct);

        var companyName = await _db.Companies.Where(c => c.Id == dep.CompanyId).Select(c => c.CompanyName).FirstOrDefaultAsync(ct) ?? "";
        return ServiceResult<DepartementReadDto>.Ok(new DepartementReadDto { Id = dep.Id, DepartementName = dep.DepartementName, CompanyId = dep.CompanyId, CompanyName = companyName, CreatedAt = dep.CreatedAt.DateTime });
    }
    ...
    public async Task<ServiceResult<DepartementReadDto>> UpdateDepartementAsync(int id, DepartementUpdateDto dto, int updatedBy, CancellationToken ct = default)
    {
        ...
        {
            var nameExists = await _db.Departements.AnyAsync(d => d.CompanyId == dep.CompanyId && d.DepartementName == dto.DepartementName && d.Id != id && d.DeletedAt == null, ct);
            if (nameExists)
                return ServiceResult<DepartementReadDto>.Fail("Un département avec ce nom existe déjà dans cette société");
        ...
        {
            var companyExists = await _db.Companies.AnyAsync(c => c.Id == dto.CompanyId.Value && c.DeletedAt == null, ct);
            if (!companyExists)
                return ServiceResult<DepartementReadDto>.Fail("La société spécifiée n'existe pas");
            var hasEmployees = await _db.Employees.AnyAsync(e => e.DepartementId == id && e.DeletedAt == null, ct);
            if (hasEmployees)
                return ServiceResult<DepartementReadDto>.Fail("Impossible de changer la société car le département contient des employés");
            var nameExistsInNew = await _db.Departements.AnyAsync(d => d.CompanyId == dto.CompanyId.Value && d.DepartementName == dep.DepartementName && d.Id != id && d.DeletedAt == null, ct);
            if (nameExistsInNew)
                return ServiceResult<DepartementReadDto>.Fail("Un département avec ce nom existe déjà dans la société cible");
        ...
        if (oldName != dep.DepartementName)
            await _companyEventLog.LogEventAsync(dep.CompanyId, "DepartementName_Changed", oldName, null, dep.DepartementName, dep.Id, updatedBy, ct);

        if (oldCompanyId != dep.CompanyId)
        {
            await _companyEventLog.LogEventAsync(oldCompanyId, "Departement_Unlinked", dep.DepartementName, dep.Id, null, null, updatedBy, ct);
            await _companyEventLog.LogEventAsync(dep.CompanyId, "Departement_Linked", null, null, dep.DepartementName, dep.Id, updatedBy, ct);
        }
    ...
    public async Task<ServiceResult<IEnumerable<JobPositionReadDto>>> GetAllJobPositionsAsync(CancellationToken ct = default)
    {
        var list = await _db.JobPositions
            .Where(jp => jp.DeletedAt == null)
            .Include(jp => jp.Company)
            .OrderBy(jp => jp.Name)
            .Select(jp => new JobPositionReadDto { Id = jp.Id, Name = jp.Name, CompanyId = jp.CompanyId, CompanyName = jp.Company != null ? jp.Company.CompanyName : null, CreatedAt = jp.CreatedAt.DateTime })
            .ToListAsync(ct);
    ...
    {
        var jp = await _db.JobPositions
            .Where(x => x.Id == id && x.DeletedAt == null)
            .Include(x => x.Company)
            .Select(x => new JobPositionReadDto { Id = x.Id, Name = x.Name, CompanyId = x.CompanyId, CompanyName = x.Company != null ? x.Company.CompanyName : null, CreatedAt = x.CreatedAt.DateTime })
            .FirstOrDefaultAsync(ct);
        return jp == null ? ServiceResult<JobPositionReadDto>.Fail("Poste non trouvé") : ServiceResult<JobPositionReadDto>.Ok(jp);
    }

    public async Task<ServiceResult<IEnumerable<JobPositionReadDto>>> GetJobPositionsAsync(int companyId, CancellationToken ct = default)
    {
        var companyExists = await _db.Companies.AnyAsync(c => c.Id == companyId && c.DeletedAt == null, ct);
        if (!companyExists)
    ...
    public async Task<ServiceResult<JobPositionReadDto>> CreateJobPositionAsync(JobPositionCreateDto dto, int createdBy, CancellationToken ct = default)
    {
        var companyExists = await _db.Companies.AnyAsync(c => c.Id == dto.CompanyId && c.DeletedAt == null, ct);
        if (!companyExists)
            return ServiceResult<JobPositionReadDto>.Fail("Société non trouvée");
        var nameExists = await _db.JobPositions.AnyAsync(jp => jp.Name == dto.Name && jp.CompanyId == dto.CompanyId && jp.DeletedAt == null, ct);
        if (nameExists)
            return ServiceResult<JobPositionReadDto>.Fail("Un poste avec ce nom existe déjà pour cette société");

        var jp = new JobPosition { Name = dto.Name, CompanyId = dto.CompanyId, CreatedBy = createdBy };
        _db.JobPositions.Add(jp);
        await _db.SaveChangesAsync(ct);

        await _companyEventLog.LogEventAsync(jp.CompanyId, "JobPosition_Created", null, null, jp.Name, jp.Id, createdBy, ct);

        var companyName = await _db.Companies.Where(c => c.Id == jp.CompanyId).Select(c => c.CompanyName).FirstOrDefaultAsync(ct);
        return ServiceResult<JobPositionReadDto>.Ok(new JobPositionReadDto { Id = jp.Id, Name = jp.Name, CompanyId = jp.CompanyId, CompanyName = companyName, CreatedAt = jp.CreatedAt.DateTime });
    }
    ...
    public async Task<ServiceResult<JobPositionReadDto>> UpdateJobPositionAsync(int id, JobPositionUpdateDto dto, int updatedBy, CancellationToken ct = default)
    {
        ...
        {
            var nameExists = await _db.JobPositions.AnyAsync(x => x.Name == dto.Name && x.CompanyId == jp.CompanyId && x.Id != id && x.DeletedAt == null, ct);
            if (nameExists)
                return ServiceResult<JobPositionReadDto>.Fail("Un poste avec ce nom existe déjà pour cette société");
            jp.Name = dto.Name;
        }
        ...
        await _companyEventLog.LogEventAsync(jp.CompanyId, "JobPosition_Updated", oldName, jp.Id, jp.Name, jp.Id, updatedBy, ct);
    ...
    public async Task<ServiceResult<IEnumerable<ContractTypeReadDto>>> GetContractTypesAsync(int companyId, CancellationToken ct = default)
    {
        var list = await _db.ContractTypes.Where(ct2 => ct2.CompanyId == companyId && ct2.DeletedAt == null)
            .Select(c => new ContractTypeReadDto { Id = c.Id, ContractTypeName = c.ContractTypeName, CompanyId = c.CompanyId, LegalContractTypeId = c.LegalContractTypeId })
            .ToListAsync(ct);
        return ServiceResult<IEnumerable<ContractTypeReadDto>>.Ok(list);
    }

    public async Task<ServiceResult<ContractTypeReadDto>> GetContractTypeByIdAsync(int id, CancellationToken ct = default)
    {
        var c = await _db.ContractTypes.FirstOrDefaultAsync(ct2 => ct2.Id == id && ct2.DeletedAt == null, ct);
        return c == null ? ServiceResult<ContractTypeReadDto>.Fail("Type de contrat introuvable.") : ServiceResult<ContractTypeReadDto>.Ok(new ContractTypeReadDto { Id = c.Id, ContractTypeName = c.ContractTypeName, CompanyId = c.CompanyId, LegalContractTypeId = c.LegalContractTypeId });
    }

    public async Task<ServiceResult<ContractTypeReadDto>> CreateContractTypeAsync(ContractTypeCreateDto dto, int createdBy, CancellationToken ct = default)
    {
        var ct2 = new ContractType { ContractTypeName = dto.ContractTypeName, CompanyId = dto.CompanyId, LegalContractTypeId = dto.LegalContractTypeId, StateEmploymentProgramId = dto.StateEmploymentProgramId, CreatedBy = createdBy };
    ...
    public async Task<ServiceResult<IEnumerable<WorkingCalendarReadDto>>> GetAllWorkingCalendarsAsync(CancellationToken ct = default)
    {
        var list = await _db.WorkingCalendars
            .AsNoTracking()
    ...
    public async Task<ServiceResult<IEnumerable<WorkingCalendarReadDto>>> GetWorkingCalendarAsync(int companyId, CancellationToken ct = default)
    {
        var companyExists = await _db.Companies.AnyAsync(c => c.Id == companyId && c.DeletedAt == null, ct);
        if (!companyExists)
    ...
    public async Task<ServiceResult<WorkingCalendarReadDto>> UpsertWorkingCalendarDayAsync(WorkingCalendarCreateDto dto, int userId, CancellationToken ct = default)
    {
        ...
        var duplicate = await _db.WorkingCalendars.AnyAsync(
            wc => wc.CompanyId == dto.CompanyId && wc.DayOfWeek == dto.DayOfWeek && wc.DeletedAt == null, ct);
        if (duplicate)
            return ServiceResult<WorkingCalendarReadDto>.Fail("Un calendrier existe déjà pour ce jour de la semaine dans cette société");
    ...
    public async Task<ServiceResult<WorkingCalendarReadDto>> UpdateWorkingCalendarDayAsync(int id, WorkingCalendarUpdateDto dto, int updatedBy, CancellationToken ct = default)
    {
        ...
        if (dto.DayOfWeek.HasValue && dto.DayOfWeek.Value != wc.DayOfWeek)
        {
            ...
            var dayTaken = await _db.WorkingCalendars.AnyAsync(
                x => x.CompanyId == compId && x.DayOfWeek == dto.DayOfWeek.Value && x.Id != id && x.DeletedAt == null, ct);
            if (dayTaken)
                return ServiceResult<WorkingCalendarReadDto>.Fail("Un calendrier existe déjà pour ce jour de la semaine dans cette société");
    ...
    public async Task<ServiceResult<WorkingCalendarReadDto>> GetWorkingCalendarDayByIdAsync(int id, CancellationToken ct = default)
    {
        var wc = await _db.WorkingCalendars
            .AsNoTracking()
            .Include(x => x.Company)
            .FirstOrDefaultAsync(x => x.Id == id && x.DeletedAt == null, ct);
        if (wc == null)
    ...
    public async Task<ServiceResult> DeleteWorkingCalendarDayAsync(int id, int deletedBy, CancellationToken ct = default)
    {
        var wc = await _db.WorkingCalendars.FirstOrDefaultAsync(x => x.Id == id && x.DeletedAt == null, ct);
        if (wc == null)
    ...
    {
        var types = new[] { new { id = "National", name = "National" }, new { id = "Religieux", name = "Religieux" }, new { id = "Autre", name = "Autre" } };
    ...
    public async Task<ServiceResult<IEnumerable<CompanyDocumentReadDto>>> GetAllDocumentsAsync(CancellationToken ct = default)
    {
        var list = await _db.CompanyDocuments
            .AsNoTracking()
            .Where(d => d.DeletedAt == null)
            .Include(d => d.Company)
    ...
    public async Task<ServiceResult<CompanyDocumentReadDto>> GetDocumentByIdAsync(int id, CancellationToken ct = default)
    {
        var d = await _db.CompanyDocuments
            .AsNoTracking()
            .Where(x => x.Id == id && x.DeletedAt == null)
            .Include(x => x.Company)
            .FirstOrDefaultAsync(ct);
        ...
        return ServiceResult<CompanyDocumentReadDto>.Ok(new CompanyDocumentReadDto
        {
            Id = d.Id,
            CompanyId = d.CompanyId,
            CompanyName = d.Company != null ? d.Company.CompanyName : "N/A",
    ...
    public async Task<ServiceResult<IEnumerable<CompanyDocumentReadDto>>> GetDocumentsAsync(int companyId, CancellationToken ct = default)
    {
        var companyExists = await _db.Companies.AnyAsync(c => c.Id == companyId && c.DeletedAt == null, ct);
        if (!companyExists)
    ...
    public async Task<ServiceResult<CompanyDocumentReadDto>> CreateDocumentAsync(CompanyDocumentCreateDto dto, int createdBy, CancellationToken ct = default)
    {
        var companyExists = await _db.Companies.AnyAsync(c => c.Id == dto.CompanyId && c.DeletedAt == null, ct);
        if (!companyExists)
            return ServiceResult<CompanyDocumentReadDto>.Fail("Entreprise non trouvée");

        var doc = new CompanyDocument { CompanyId = dto.CompanyId, Name = dto.Name, FilePath = dto.FilePath, DocumentType = dto.DocumentType, CreatedBy = createdBy };
        _db.CompanyDocuments.Add(doc);
        await _db.SaveChangesAsync(ct);
        var companyName = await _db.Companies.Where(c => c.Id == doc.CompanyId).Select(c => c.CompanyName).FirstOrDefaultAsync(ct) ?? "N/A";
        return ServiceResult<CompanyDocumentReadDto>.Ok(new CompanyDocumentReadDto
        {
            Id = doc.Id,
    ...
    public async Task<ServiceResult<string>> SaveFileAsync(IFormFile file, int companyId, string? documentType, CancellationToken ct = default)
    {
    ...
    public async Task<ServiceResult<(byte[] fileBytes, string contentType, string fileName)>> GetFileAsync(string filePath, CancellationToken ct = default)
    {
        if (string.IsNullOrWhiteSpace(filePath))
